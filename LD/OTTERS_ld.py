...
def thread_process(num):
    ...
    sub_outdir = os.path.join(param_dict['out_dir'], 'PLINK_blk' + str(blk))
    ots.check_path(sub_outdir)

    # generate snplist
    out_range = os.path.join(sub_outdir, 'range' + str(blk) + '.txt')

    with open(out_range, 'w') as ff:
        ff.write('%s\t%s\t%s\t%s\n' % (chr, Block.Start, Block.End, blk))

    snplist_out = os.path.join(out_dir, 'ldblk' + str(blk))
    get_snplist_cmd = ["plink --bfile " + b_path + " --extract range " +
                       out_range + " --write-snplist --out " + snplist_out]
    ...
    # calculate LD
    ldblk_out = os.path.join(sub_outdir, 'ldblk' + str(blk))
    get_ld_cmd = ["plink --bfile " + param_dict['b_path'] + " --keep-allele-order --extract range " +
                  out_range + " --r square --out " + ldblk_out + " --memory 2000 "]
    ...
    if len(snplist) > 0:
        ...
        if len(snplist) == len(ld_blk):
            ...
            out_ld = os.path.join(out_dir + 'chr' + chr + '_ldblk' + str(blk) + '.txt')
            ...
            # remove the ld file generated by PLINK
            for f in os.listdir(sub_outdir):
                os.remove(os.path.join(sub_outdir, f))
